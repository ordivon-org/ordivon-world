...
class W2LegacyResourceRetirementTests(unittest.TestCase):
    def test_pre_p4_flat_resource_state_fails_closed(self) -> None:
        journal = HostResourceTransferJournal.__new__(HostResourceTransferJournal)
        with self.assertRaisesRegex(ResourceTransferError, "pre-0.6 recovery client"):
            journal._instances({"worldResourceTransferPlanDigest": "sha256:legacy"})
