...
`WorldTaskInspector` is the bounded read-only projection over that owner state. It is deliberately an aggregator, not a second state model: Provider dispatches project their own bounded request/observation evidence, and the private typed trajectory journal projects Resource, Message and Entity plan/receipt/uncertainty evidence. The aggregator knows only the exact Host Task revision, the generic `world` namespace pointer and the four owner-local projector interfaces; it does not decode trajectory storage fields or return payload, provenance or continuity bodies. Inspection may report an outstanding owner operation such as reconciliation or exact-original retry, but every result carries `authority=not-granted-by-inspection` and `externalCurrentness=not-claimed`. There is no OwnerRegistry or shared cross-domain inspection schema.
