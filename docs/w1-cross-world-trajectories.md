...
1. Link binding replacement between durable preparation and destination admission;
2. destination Reality drift after a retained receipt;
3. destination receipt loss with native materialization still present;
4. a third materially different trajectory, such as durable Message Delivery or a non-Security physical destination;
5. multi-hop federation where each hop has independent durable admission and receipt state.
