...
| W5-B Agent Presence | active research; B0 fresh-Agent decision value verified | a fresh live Harness Agent given only historical Game/Security embodiment history safely returned `insufficient-current-evidence`; the same decision with fresh owner relation projections selected the Game Body while Security Body A was `absent-through-body`. Presence currently earns its cost only as decision-scoped epistemic evidence; B1 seeks a second active destination before any shared production contract |
