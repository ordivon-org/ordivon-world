...
| W5-B Agent Presence | active research; B0 and B1 verified, no production Presence contract | B0 shows fresh owner relation evidence changes a safe Agent body-choice from `insufficient-current-evidence` to the currently present Game Body. B1 proves Security is a second bounded subject-active destination: one exact KVM Body consumed independently bound continuity and activation media, verified subject/migration/continuity/body generation in Guest, returned a bounded result while Host retained native liveness authority, and closed with zero residuals. Security is not yet a second live-cognition destination; B2 now tests the minimal cross-domain invariant before any shared Embodiment/Presence contract |
