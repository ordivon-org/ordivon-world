...
| W5-A Agent Embodiment | active research; A0–A3 verified, no production contract | A0/A1 separated continuity integrity from ownership; A2 showed Provider attribution is not persistent Agent embodiment; A3 used current Harness live DeepSeek cognition plus a detached Game destination-admission candidate to prove two exact action-scoped subject × cognition × Actor bindings, two verified `actor_moved` effects, cross-cognition rejection, digest-laundering rejection and fresh recovery. One bounded embodiment occurrence is proven; persistent embodiment and current Presence remain unproven |
...
- Entity Migration materializes a continuity carrier; it still does not prove Guest-side subject activation or current Presence. W5-A A3 separately proves bounded action-scoped subject/cognition activation through a Game Actor, not persistent Presence or Security Guest activation.
