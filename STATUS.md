...
The active source boundary consists of the Host-facing Cloudflare adapter, production Resource Transfer, Message Delivery and Entity Migration contracts, Workstation-owned foreign-egress capability projection, the narrow Agent-facing Effect Path Query, Cloudflare Worker/provider operations, and private network condition tools. There is no independent World service, global capability registry or provider router.
...
| W-X1 Foreign Egress Capability | verified owner-observed projection | Workstation Surfpath observation `sha256:327472...` projected one explicit Agent-selected JP/OpenVPN path into `ForeignEgressCapability`; the projection preserved semantic path/destination evidence and a 180-second owner freshness horizon while stripping route/auth/provider mechanics. Activation authority remained `ordivon.workstation.surfpath` and required owner revalidation. |
...
| W-X3 Effect Path Query | P1 verified; shared informational projection, not generic capability ownership | Surfpath and a request-scoped Cloudflare fixed-target connector reached the same OpenAI GET through materially different owner domains. The old Cloudflare success later had DNS/route/Worker all 404. A new connector's three resources were all owner-observed present, yet the first exact effect returned HTTP 522; the same resource identity then returned OpenAI HTTP 401, followed by verified cleanup to absent. `EffectPathQuery` therefore preserves owner authority/currentness/usability as separate coordinates, keeps Surfpath's native `freshUntil` while Cloudflare validity remains null, exposes no recommendation/rank, sets `selectionAuthority=agent`, and requires exact candidate selection plus owner revalidation. |
...
| cross-language JSON Schema | verified | twenty-nine packaged Draft 2020-12 Schemas, including the narrow `effect-path-query`; provider fixtures plus Game-produced Resource, Message and Entity departure authority documents validated across TypeScript/Python |
...
- The production remote-provider adapter set contains only Cloudflare. `EffectPathQuery` can compare Cloudflare evidence with Workstation Surfpath evidence, but it is an informational choice projection rather than a provider broker, generic Capability owner or automatic router.
