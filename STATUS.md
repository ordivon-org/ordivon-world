...
| W5-A Agent Embodiment | active research; A0–A4-P0 verified, no production contract | A0–A3 prove owner-separated, action-scoped subject × cognition × Body occurrence with live Harness cognition and native Game effects. A4-P0 shows current Presence cannot be inferred from that history: a Game Body may remain active while the old subject binding is stale (`UNKNOWN`), while a Security KVM historical materialization receipt may survive after the physical carrier is cleanly absent. Presence is currently a scope-bound owner-observed relation hypothesis, not a global registry |
