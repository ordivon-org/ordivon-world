...
use crate::model::{EdgeEvent, EdgeSnapshot, HealthState, ServiceState};
...
impl Store {
    ...
    pub fn latest_snapshot(&self) -> Result<Option<EdgeSnapshot>> {
        let connection = self
            .connection
            .lock()
            .map_err(|_| anyhow::anyhow!("edge store mutex poisoned"))?;
        ...
        for payload in payloads {
            if let Ok(snapshot) = serde_json::from_str::<EdgeSnapshot>(&payload?) {
...
fn derive_events(previous: Option<&EdgeSnapshot>, current: &EdgeSnapshot) -> Vec<EdgeEvent> {
...
mod tests {
    ...
    fn snapshot(health: HealthState, connected: bool) -> EdgeSnapshot {
        EdgeSnapshot {
            schema_version: 1,
            observed_at: Utc::now(),
            health,
